...
public class TitleManager : MonoBehaviour
{
    ...
    {
        blurEffectManager.ShowTitleAndApplyBlur();
        photonManager.ConnectToPhotonNetwork();  // 포톤 네트워크 접속 시도
        yield return new WaitUntil(() => PhotonNetwork.IsConnected);  // 접속 완료까지 대기

        blurEffectManager.ShowTitleAndApplyBlur();  // 타이틀 텍스트와 블러 효과 적용
        blurEffectManager.StartTitleAndBlurFadeOut(OnFadeOutComplete);  // 페이드 아웃 시작 후 콜백 함수 호출
    }
